...
| FR-010 | Done | `src/manifest.rs` parses + validates per `docs/manifest-schema.md`; `dispatch_register` invokes `Manifest::load` before binding, surfacing `manifest_invalid` / `manifest_not_found` on failure. JSON Schema artifact published at `schema/remo-broker.v1.json`, regenerated + snapshot-tested via the `schema-gen` Cargo feature (CI runs `cargo test --all-features` so drift is caught on every PR). |
...
| ID | Status | Notes |
|---|---|---|
| NFR-001 | Done | `benches/latency.rs::bench_warm_cache_get` measures end-to-end warm-cache `get` round-trip through the project socket. Latest local run: **~87 µs mean** (target ≤5 ms = 57× headroom). p99 is not directly reported by Criterion's default mode; the mean + tight confidence intervals are well within budget. |
| NFR-002 | Done | `benches/latency.rs::bench_cold_get` measures broker overhead on the cache-miss path with the in-process `plain` provider (so backend RTT is ~microseconds; the measurement isolates broker overhead). Latest local run: **~86 µs mean** broker overhead — well under the 20 ms budget. Real backends add their RTT on top. |
...
| SC-002 | Done (smoke) / Pending (full) | `examples/soak.rs` is the harness. CI's `harnesses` job runs a 5-second smoke version on every PR (5 projects × 2 workers × 10 Hz, ~500 ops). The full SC-002 workload (50 sockets × 10 Hz × 1 hour, ~1.8 M ops) is wired into `.github/workflows/soak.yml`, scheduled nightly at 04:00 UTC and manually triggerable. Failure conditions: any worker panic, error rate >1%, RSS > cap, or audit-line count below the expected `3/5 × ops` floor. |
| SC-003 | Done (smoke) / Pending (extended) | `examples/killtest.rs` is the harness. CI's `harnesses` job runs 5 rounds × 50 ops × 100 ms kill-delay on every PR. Each round asserts (a) no tripwire plaintext leaks under the daemon's writable paths after SIGKILL, (b) the audit log remains parseable JSONL, (c) the next restart cleans the stale socket and re-registers cleanly. Extended scenarios (longer runs, killing under register/reload rather than get, multiple concurrent victims) are follow-up work. |
...
1. **Project-socket group ownership** (FR-007's "additional group access configured per the project's devcontainer ownership"). The unit runs the daemon as `User=remo-broker, Group=remo-broker` so sockets are `remo-broker:remo-broker` by default; the devcontainer's effective UID/GID needs membership in the `remo-broker` group, OR the broker needs to `chown` the socket to a per-project group after bind. Decide and wire. (Blocked on Remo OQ in the handoff doc.)
2. **NFR-005 binary-size reduction** (~32 MiB → 15 MiB target). Options: (a) feature-gate the fnox-core providers we don't need (upstream change), (b) build a slim fnox-core fork, (c) accept the larger size and revise the NFR. Measured + tracked by `ci/measure_nfr.sh`; currently emits a `::warning::` rather than failing the job.
3. **Full cargo-fuzz harness for SC-001** (24-hour campaign). Smoke fuzz via `proto::smoke_fuzz` (40 000 iterations per `cargo test`) is in place; the full requirement needs nightly Rust + a scheduled long-running workflow.
4. **SC-005 red-team harness** — hostile devcontainer attempting allowlist bypass, brute-force secret names, cache exhaustion, cross-project escalation. Builds on the soak/killtest infrastructure; the broker's denial paths are wired so this is a real exercise. SC-006 cross-repo CI still moves with the Remo team's adoption.
