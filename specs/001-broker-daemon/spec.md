...
**Created**: 2026-05-24
**Status**: In Progress
**Last Updated**: 2026-05-24 (commit `2c55d6f`; 126 tests passing, clippy + cargo deny + systemd-analyze verify + NFR measurements green)
...
| ID | Status | Notes |
|---|---|---|
| NFR-001 | Unverified | Cache lookup is `Mutex::lock` + `HashMap::get` + a single `String::clone` for the value — comfortably ≤5 ms p99 on any modern Linux box, but no `criterion` harness yet. Lands with the SC-002 soak. |
...
3. **NFR-001/002 latency benchmarks** (warm cache p99 ≤5 ms; cold ≤backend + 20 ms broker). Needs `criterion` harness; lands with SC-002 soak infrastructure.
4. **Test harnesses for SC-001/002/003/005** (NDJSON-parser fuzz, 1-hour 50×10 Hz soak, killtest, red-team). SC-005 can now exercise the real backend path; the others remain infrastructure work. SC-006 cross-repo CI moves with the Remo team's adoption.
